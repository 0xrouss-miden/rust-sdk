...
* [BREAKING][removal][test] Removed `FeeFunder::flush`, `TestClient::flush_funder` and `ClientConfig::flush_funder`. The funding service answers only once its note has committed, so there is nothing left in flight for a run to wait on. Added `TestClient::consume_notes_and_wait`, which pairs `consume_notes` with the commit wait nearly every caller needed ([#2573](https://github.com/0xMiden/rust-sdk/issues/2573)).
