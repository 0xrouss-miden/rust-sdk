...
* [FEATURE][rust] `Client::register_account` records the account in the store as accepted by the network allowlist, so creating that account never asks the network again. `Store` gained `mark_account_allowlisted` and `is_account_allowlisted`, both with default implementations over the settings store, so no store implementation has to change ([#2550](https://github.com/0xMiden/rust-sdk/pull/2550)).
